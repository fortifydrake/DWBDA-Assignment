# Data Warehousing and Big Data Analytics (DWBDA)

### Name: 
Aditya Prasad
### Registration no:
2020400333

## Assignment 1 : 
Apply MBA on real world data using python . 

## Assignment 2: 
Apply map reduce framework to calculate the number of Students in each categories ( S,A,B,C,D,E,F) use the previous semester final exam results às the sample data and calculate for all the subjects . 


## Assignment 3: 
Take the sales data from anywhere, and write the spark program to calculate no. of product sold in each category

# NOTE:
1. transaction_data.csv, data used for assignment 1 and 3 is inside archive.zip 
2. Data for assignment 2 is previous semester grade marks for all subjects like DAA, DBMS, PS&SP, Programming in Java, Introduction to Cyber Security, PBL, OOPS using JAVA Lab and DBMS lab